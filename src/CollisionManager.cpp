...
bool CollisionManager::squaredRadiusCheck(GameObject* object1, GameObject* object2)
{
	...
	if (CollisionManager::squaredDistance(P1, P2) < (halfHeights * halfHeights)) {
		if (!object2->getIsColliding()) {
			...
			case CASH:
				std::cout << "Collision with Cash!" << std::endl;
				TheSoundManager::Instance()->playSound("cash", 0);
				if (object1->getType() == PLAYER) {
					ScoreBoardManager::Instance()->setPlayerScore(ScoreBoardManager::Instance()->getPlayerScore() + 500);
				}
				if (object1->getType() == ENEMY) {
					ScoreBoardManager::Instance()->setEnemyScore(ScoreBoardManager::Instance()->getEnemyScore() + 2000);
				}
				object2->clean();
				break;
			case PLAYER:
			...
			case ENEMY:
				...
				break;
			case DYNAMITE:
				std::cout << "Collision with DYNAMITE!" << std::endl;
				TheSoundManager::Instance()->playSound("explosion", 0);
				if (object1->getType() == PLAYER) {
					ScoreBoardManager::Instance()->setPlayerHealth(ScoreBoardManager::Instance()->getPlayerHealth() - 1);
				}
				if (object1->getType() == ENEMY) {
					ScoreBoardManager::Instance()->setEnemyHealth(ScoreBoardManager::Instance()->getEnemyHealth() - 1);
				}
				object2->clean();
			default:
